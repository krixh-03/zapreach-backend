  ...
  const sendEmail = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setIsLoading(false); 
        setToastMessage(`Error: ${textResponse || "Unknown error"} (Status: ${res.status})`);
        setToastType("error");
        setToastVisible(true);
      }
    } catch (error: any) {
    ...
    setTimeout(() => {setToastVisible(false)}, 5000); // Hide the toast after 3 seconds
  };
